...
namespace NameGenerator {
  ...
  export class Elite implements NameGenerator {
      ...
      randName(): string {
        const pair1 = 2 * Math.floor(this.rng.rand() * (this.pairs.length / 2));
        const pair2 = 2 * Math.floor(this.rng.rand() * (this.pairs.length / 2));
        const pair3 = 2 * Math.floor(this.rng.rand() * (this.pairs.length / 2));
        const pair4 = 2 * Math.floor(this.rng.rand() * (this.pairs.length / 2));
  ...
  export class Markov implements NameGenerator {
      ...
      produce(prev: string): string {
        ...
        while (prev !== '$') {
          ...
          const total: number = this.transitions[key];
          const random = Math.floor(total * this.rng.rand());
          let k = 0;
          ...
          for (const property in this.transitions) {
            if (this.transitions.hasOwnProperty(key)
              ...
              if (k >= random) {
                const next = property.substr(prev.length);
                result += next;
                prev = prev.substr(-this.order + 1) + next;
                break;
              }
